  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics((prev) => ({
        ...
        memoryUsed: Math.max(1.5, Math.min(7.5, prev.memoryUsed + (Math.random() - 0.5) * 0.3)),
        networkLatency: Math.max(5, Math.min(100, prev.networkLatency + (Math.random() - 0.5) * 10)),
        uptime: prev.uptime + 2,
        errorCount: prev.errorCount + (Math.random() < 0.05 ? 1 : 0),
        healthScore: Math.max(
          60,
          Math.min(
            100,
            100 -
              (prev.cpuUsage > 80 ? 10 : 0) -
              (prev.memoryUsed / prev.memoryTotal > 0.85 ? 10 : 0) -
              (prev.networkLatency > 50 ? 10 : 0) -
              (prev.errorCount > 10 ? 15 : prev.errorCount > 5 ? 10 : 0)
          )
        ),
      }));
    }, 2000);
  ...
  return (
    <Card className="p-4 bg-gradient-to-br from-card/80 to-card/60 dark:from-card/80 dark:to-card/60 border-border/30 dark:border-border backdrop-blur-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="font-semibold text-lg">System Diagnostics</h3>
